...
export const Utils = {
  ...
  storeTokenResponse: async (tokenResponse: any) => {
    Object.keys(tokenResponse).forEach(async t => {
      await AsyncStorage.setAsyncItem(t, String(tokenResponse[t]));
    });
  },
  changeStatusBar: (
  ...
  date: {
    ...
    formatTimeAGMT9: (datetime: any) =>
      `${moment
        .utc(datetime)
        .utcOffset("+09:00")
        .format("hh:mm:ss A - DD/MM/YYYY")} (GMT+9)`,
    formatTimeGMT9: (datetime: any) =>
      `${moment
        .utc(datetime)
        .utcOffset("+09:00")
        .format("hh:mm:ss A")} (GMT+9)`,
  ...
  padNumber: (s: number, length = 2, char = "0") => {
  ...
  //show Error Messages Api
  showErrorMessages: (errorCode: string) => {
    const error = DATA_CONSTANT.ERROR_CODE_API.find(item =>
      item.value.includes(errorCode),
    );
    return error?.message ?? "error.errorServer";
  },
};
