...
export const LaunchScreen: FunctionComponent = () => {
  ...
  const authenticate = async (): Promise<void> => {
    const [accessToken, anonymousId, language] = await Promise.all([
      ...
      AsyncStorage.getAsyncItem(CONSTANT.TOKEN_STORAGE_KEY.ANONYMOUS_ID),
      AsyncStorage.getAsyncItem(CONSTANT.TOKEN_STORAGE_KEY.LANGUAGE),
    ]);
    ...
    if (accessToken) {
      ...
      if (anonymousId) {
        ...
        await AsyncStorage.setAsyncItem(
